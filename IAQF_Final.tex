...
The SVB episode reveals that the cross-currency fragmentation we document originates entirely in the banking layer, not in crypto price mechanics. Circle held \$3.3B of its ${\approx}$\$40B USDC reserves (${\approx}8\%$) at SVB. When SVB failed on Friday evening March 10, those reserves became temporarily inaccessible and USDC holders had no guaranteed redemption timeline. The resulting confidence shock---not any on-chain mechanism---drove the \$0.87 de-peg and the $+320$ bps dispersion blowout we document. The peg did not fully recover until March 17, after the Federal Reserve announced emergency deposit guarantees and Circle secured alternative banking arrangements.

The GENIUS Act (signed July 2025) targets this exact failure chain through four reinforcing provisions, each mapping to a specific channel in our results. First, \emph{reserve composition requirements} mandate one-to-one backing exclusively in Treasury bills, FDIC-insured deposits, central bank reserves, and Treasury-backed repos. Table~\ref{tab:genius_cf} formalizes the counterfactual: had Circle been required to hold reserves in T-bills and central bank facilities rather than uninsured bank deposits, the \$3.3B at SVB ($8.25\%$ of total reserves) would have been in instruments immune to bank failure. The lock-up that caused the de-peg could not have occurred, and our estimated crisis $D_t$ of $+319.97$ bps would have remained at the pre-crisis level of $+0.12$ bps---a $99.9\%$ reduction. Second, \emph{on-demand par redemption within two business days} eliminates the panic amplification loop: during the SVB weekend, holders could not redeem, turning a reserve-access problem into the market-wide pricing dislocation we document. Third, \emph{bankruptcy super-priority} directly caps the $B_t$ tail risk we observe expanding beyond $\pm100$ bps during the crisis (Figure~\ref{fig:bdist})---the crisis P99 of $+81.4$ bps would revert to pre-crisis $+13.6$ bps.

\input{tables/genius_counterfactual.tex}

Fourth, \emph{transparency} closes the information channel. The GENIUS Act mandates monthly public disclosure of reserve composition with CEO/CFO certification. During the SVB crisis, traders did not know USDC's bank exposure until Circle's Friday-night announcement; this asymmetry drove the sharp divergence between USDC ($D_t=+320$ bps) and USDT (safe-haven premium $+20.7$ bps)---exactly the flight-to-quality rotation our Roll spread, Amihud ILLIQ, and volume evidence document.
...
These findings converge on a single policy insight: the GENIUS Act's reserve composition, redemption guarantee, transparency, and bankruptcy super-priority provisions target the exact failure chain---reserve inaccessibility, information asymmetry, confidence collapse, and peg break---that produced the fragmentation we document. A natural next step is a post-GENIUS replication study during the next stress event, testing whether the Act's provisions compress $D_t$ and $B_t$ tails, attenuate flight-to-quality rotation, and narrow cross-venue arbitrage windows relative to the SVB baseline. The open question is whether reserve-layer reforms alone are sufficient, or whether additional market-structure measures---cross-chain interoperability mandates, consolidated cross-venue price feeds, or unified clearing for stablecoin-settled trades---are needed to eliminate the residual spatial fragmentation that persists even in fiat channels.
