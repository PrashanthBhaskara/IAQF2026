...
# Forward fill prices up to 5 minutes
prices = pd.DataFrame({
    name: extract_col(df, 'close', full_index).ffill(limit=5)
...
# 3. Calculate Real Fiat-Pegged Basis
# Formula: b_Q(t) = log(P_BTC/Q * P_Q/USD) - log(P_BTC/USD)
basis = pd.DataFrame(index=full_index)

# KRAKEN: BTC/USDC vs BTC/USD (Using exact Kraken USDC/USD pair)
# basis_usdc_kraken = log(P_BTC/USDC * P_USDC/USD) - log(P_BTC/USD)
basis['basis_usdc_kraken'] = (
    np.log(prices['kraken_btcusdc'] * prices['kraken_usdcusd']) - np.log(prices['kraken_btcusd'])
) * 10000 # in bps

# KRAKEN: BTC/USDT vs BTC/USD (Using exact Kraken USDT/USD pair)
basis['basis_usdt_kraken'] = (
    np.log(prices['kraken_btcusdt'] * prices['kraken_usdtusd']) - np.log(prices['kraken_btcusd'])
) * 10000

# COINBASE: BTC/USDT vs BTC/USD (Using exact Coinbase USDT/USD pair)
